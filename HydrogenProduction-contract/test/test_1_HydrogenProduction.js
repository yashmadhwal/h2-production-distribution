...
describe("Basic Smart Contract Testing", function () {
    ...
    describe('Production Phase', () => {
        ...
        it('Only the new entities can set new requirements to mint', async function () {
            // changing roles
            await h2p.setWaterProvider(tempAddress[1].address);
            expect (await h2p.WATER_PROVIDER()).equal(tempAddress[1].address);
            ...
            await expect(h2p.connect(waterSupplier).setElectricityRequirement(1000)).to.revertedWith('Not Authorised Access');
            await expect(h2p.connect(renewables).setWaterRequirement(1000)).to.revertedWith('Not Authorised Access');

            await expect(h2p.connect(tempAddress[3]).setElectricityRequirement(0)).to.revertedWith('Minimum amount cannot be zero');
            await expect(h2p.connect(tempAddress[3]).setWaterRequirement(0)).to.revertedWith('Minimum amount cannot be zero');

            await h2p.connect(tempAddress[3]).setElectricityRequirement(10000);
            await h2p.connect(tempAddress[3]).setWaterRequirement(1000000);
            
            // verifying
            expect(await h2p.electricityRequired()).to.equal(10000)            
            expect(await h2p.waterRequired()).to.equal(1000000)                 
